#!/usr/bin/env python3
"""Prepare, preflight, or execute one generic LF-021 collector-v5 tranche."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Create an immutable collector-v5 tranche config from a bound v2/v3 "
    ...
    try:
        if args.prepare:
            ...
            path, digest = derive_research_collection_v5_config(
        ...
        if args.config is None:
            raise ResearchCollectionV5Error("--preflight and --execute require --config")
        config_path = args.config
        if not config_path.is_absolute():
            config_path = paths.root / config_path
        loaded = load_research_collection_v5(config_path, repo_root=paths.root)
        if args.preflight:
            report_path, report_hash = write_preflight_report_v5(
                loaded,
                repo_root=paths.root,
            )
    ...
    except (ResearchCollectionV5Error, ValueError, OSError) as exc:
        print(f"FAILED: {exc}")
        return 1
