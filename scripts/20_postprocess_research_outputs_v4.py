#!/usr/bin/env python3
"""Process or replay one completed LF-021 collector-v3 operational tranche."""
...
from leanfaith.generation.research_collection_v3 import ResearchCollectionV3Error
from leanfaith.generation.research_postprocess import ResearchPostprocessError
from leanfaith.generation.research_postprocess_v3 import ResearchPostprocessV3Error
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "unresolved REVIEW-only records for one exact Algebra or cross-domain "
            "collector-v3 tranche."
        )
    )
    ...
    parser.add_argument(
        ...
        help="frozen collector-v3 config used to reproduce plan.json",
    )
    parser.add_argument(
        "--mathlib-project-dir",
        type=Path,
    ...
    parser.add_argument(
        "--verify-only",
        action="store_true",
        help="verify an existing postprocess_v4 bundle without Lean execution",
    )
    ...
    try:
        ...
        if args.verify_only:
            manifest = verify_research_postprocess_v4(loaded)
        else:
            ...
            try:
                result = run_research_postprocess_v4(
                    loaded,
                    backend=backend,
                )
            finally:
            ...
            if manifest != result.manifest:
                raise ResearchPostprocessV4Error(
                    "immediate v4 replay differs from the written manifest"
                )
    except (
    ...
    print(f"shared_processing_input_binding_hash={manifest.shared_processing_input_binding_hash}")
    print(f"tranche_id={manifest.tranche_id}")
    print(f"pool_dialect={manifest.pool_dialect}")
    print(f"pool_source={manifest.pool_source}")
