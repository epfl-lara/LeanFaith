#!/usr/bin/env python3
"""Evaluate the problem-aware LF-021 tranche expansion amendment."""
...
from leanfaith.generation.tranche_expansion_v2 import (
    TrancheExpansionV2Error,
    run_tranche_expansion_v2,
)

_DEFAULT_POLICY = Path("configs/generation/lf021_tranche_expansion_v2.yaml")
_DEFAULT_OUTPUT = Path("reports/generation/lf021_tranche_expansion_v2")


def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Choose the next frozen LF-021 tranche or create an unresolved "
            "problem-group-aware prevalence frame."
        )
    )
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="immutable postprocess manifest in exact frozen tranche order",
    )
    ...
    try:
        run = run_tranche_expansion_v2(
            repo_root=paths.root,
            policy_path=policy,
            observed_manifests=manifests,
            output_root=output,
        )
    except (OSError, ValueError, TrancheExpansionV2Error) as exc:
        print(f"FAILED: {exc}")
        return 1
    ...
    print(f"problem_aware_unique_compiling={decision.counts.unique_compiling_count}")
