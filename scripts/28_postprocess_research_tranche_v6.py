#!/usr/bin/env python3
"""Run or verify postprocess-v6 over a completed collector-v5 tranche."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Postprocess one completed collector-v5 tranche through the "
    ...
    try:
        loaded = load_research_postprocess_v6(
        ...
        if args.verify_only:
            manifest = verify_research_postprocess_v6(loaded)
        else:
            ...
            try:
                result = run_research_postprocess_v6(loaded, backend=backend)
            finally:
                backend.close()
            manifest = verify_research_postprocess_v6(loaded)
            if manifest != result.manifest:
                raise ResearchPostprocessV6Error(
                    "immediate v6 replay differs from the written manifest"
                )
    except (ResearchPostprocessV6Error, ValueError, OSError) as exc:
        print(f"FAILED: {exc}")
        return 1
