"""LF-018 N07 same-context Lean re-elaboration and structural audit."""
...
def backend(tmp_path_factory: pytest.TempPathFactory) -> Iterator[LeanInteractBackend]:
    instance = LeanInteractBackend(
        BackendSettings(
            ...
            raw_response_dir=tmp_path_factory.mktemp("n07_raw"),
        )
    )
    yield instance
    instance.close()
...
def _theorem(
    ...
    *,
    ...
    return TheoremRecord(
        ...
        source="n07_live_fixture",
        source_revision="n07_literal_bound_v1",
        context_id=_CTX,
...
@pytest.mark.parametrize(
    ("name", "source_proposition", "source_literal", "target_literal"),
    [
        ("n07_live_right_zero", "n ≤ 0", "0", "1"),
        ("n07_live_left_one", "1 < n", "1", "0"),
        ("n07_live_right_one", "n ≥ 1", "1", "0"),
        ("n07_live_left_zero", "0 > n", "0", "1"),
    ],
)
def test_n07_candidate_reelaborates_with_exact_literal_delta_but_stays_unresolved(
    ...
    source_code = f"theorem {name} (n : Nat) : {source_proposition} := by sorry"
    source_id = make_id("thm", {"n07_live": name, "side": "source"})
    ancestry_id = make_id("anc", {"n07_live": name})
    source = _theorem(
        source_code,
    ...
    rule = N07LiteralBoundRule.from_repository(registry_hash="7" * 64)
    (draft,) = rule.generate(source, source_representation, 23)

    candidate_id = make_id("thm", {"n07_live": name, "draft": draft.draft_id})
    candidate = _theorem(
    ...
    source_atoms = Counter(source_representation.semantic_atoms or ())
    candidate_atoms = Counter(candidate_representation.semantic_atoms or ())
    removed = source_atoms - candidate_atoms
    added = candidate_atoms - source_atoms
    assert set(removed) == {f"lit:nat:{source_literal}"}
    assert set(added) == {f"lit:nat:{target_literal}"}
    assert removed[f"lit:nat:{source_literal}"] == added[f"lit:nat:{target_literal}"]
    assert removed[f"lit:nat:{source_literal}"] > 0
