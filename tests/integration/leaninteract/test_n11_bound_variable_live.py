"""Live LeanInteract smoke for LF-034 N11's single-bvar certificate."""
...
_SOURCE = "theorem n11_live (x y : Nat) : x = y := by sorry"


def _source_theorem() -> TheoremRecord:
    theorem_id = make_id("thm", {"n11_live": "source"})
    ancestry_id = make_id("anc", {"n11_live": "source"})
    return theorem_record(
        ...
        source="fixtures",
        context_id=_CTX,
        declaration_name="n11_live",
        declaration_full_name="n11_live",
...
def test_n11_live_candidate_elaborates_and_has_one_certified_bvar_delta(
    tmp_path: Path,
) -> None:
...
def test_n11_live_pooled_materializer_creates_only_provisional_variant(
    tmp_path: Path,
) -> None:
    backend = LeanInteractBackend(
        BackendSettings(
