"""Live LeanInteract smoke for LF-034 N17's exact role-argument swap."""
...
from leanfaith.transforms.v2_d0_n17_runtime import build_v2_d0_n17_runtime
...
_SOURCE = "theorem n17_live (p q : Nat) : p > q := by sorry"
_CANDIDATE = "theorem n17_live (p q : Nat) : q > p := by sorry"


def _source_theorem() -> TheoremRecord:
    theorem_id = make_id("thm", {"n17_live": "source"})
    ancestry_id = make_id("anc", {"n17_live": "source"})
    return theorem_record(
        ...
        source="fixtures",
        context_id=_CTX,
        declaration_name="n17_live",
        declaration_full_name="n17_live",
...
def test_n17_live_pooled_materializer_certifies_only_provisional_variant(
    tmp_path: Path,
) -> None:
    ...
    try:
        ...
        rule = N17RoleArgumentRule(
            generation_config_hash="d" * 64,
            candidate_pool="n17_live_fixture",
        )
        (draft,) = rule.generate(source, source_representation, seed=17)
        ...
        assert result.profile_id == "deterministic_v2_d0_n17_experimental"
        assert result.rule_id == "n17_role_sensitive_arguments"
