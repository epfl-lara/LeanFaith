"""Live LeanInteract smoke for LF-034 N13's witness-dependency certificate."""
...
from leanfaith.transforms.v2_d0_n13_runtime import build_v2_d0_n13_runtime
...
_SOURCE = "theorem n13_live : ∀ x : Nat, ∃ y : Nat, x = y := by sorry"


def _source_theorem() -> TheoremRecord:
    theorem_id = make_id("thm", {"n13_live": "source"})
    ancestry_id = make_id("anc", {"n13_live": "source"})
    return theorem_record(
        ...
        source="fixtures",
        context_id=_CTX,
        declaration_name="n13_live",
        declaration_full_name="n13_live",
...
def test_n13_live_pooled_materializer_certifies_only_provisional_variant(
    tmp_path: Path,
) -> None:
    ...
    try:
        ...
        rule = N13WitnessDependencyRule(
            ...
            candidate_pool="n13_live_fixture",
        )
        (draft,) = rule.generate(source, source_representation, seed=7)
        assert draft.candidate_code == (
            "theorem n13_live : ∃ y : Nat, ∀ x : Nat, x = y := by sorry"
        )
        ...
        assert result.profile_id == "deterministic_v2_d0_n13_experimental"
        assert result.rule_id == "n13_witness_dependency"
