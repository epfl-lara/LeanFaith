...
def test_full_launcher_rejects_partial_existing_root(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    spec = _spec(tmp_path)
    ...
    monkeypatch.setattr(
        launcher,
        "_validate_full_root",
        lambda **_kwargs: (_ for _ in ()).throw(CompositionFullLaunchError("partial")),
    )
    executor = Executor()
    with pytest.raises(CompositionFullLaunchError, match="partial"):
        run_composition_full_scale(
            code_root=tmp_path / "unused-code",
            expected_commit="a" * 40,
            seed_dir=tmp_path / "unused-seed",
