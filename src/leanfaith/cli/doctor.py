...
def canonical_environment_lock() -> EnvironmentLock:
    """The Phase 0 lock derived from pinned code constants (§6.2, ADR-0001)."""
    return EnvironmentLock(
        environment_schema_version=1,
        python=PythonLock(version="3.12"),
        lean_interact=LeanInteractLock(
            version=PINNED_VERSION,
            advertised_lean_min=ADVERTISED_LEAN_MIN,
            advertised_lean_max=ADVERTISED_LEAN_MAX,
            repl_fork=EXPECTED_REPL_FORK,
        ),
...
def write_lock(paths: RepoPaths, *, force: bool = False) -> tuple[bool, str]:
    """Implement ``leanfaith doctor --write-lock`` (Phase 0 task 8).

    Writes the canonical lock if absent; with an existing lock, verifies it
    matches the canonical values and refuses to overwrite unless ``force``.
    Returns (changed, message).
    """
    canonical = canonical_environment_lock()
    lock_path = paths.configs / "environment.lock.yaml"
    if lock_path.exists() and not force:
        existing = load_environment_lock(paths)
        if existing == canonical:
            return False, f"{lock_path} exists and matches the canonical lock"
        return False, (
            f"{lock_path} exists but differs from the canonical lock; "
            "re-run with --force to overwrite (this is an ADR-0001 change)"
        )
    lock_path.parent.mkdir(parents=True, exist_ok=True)
    header = (
        "# Executable environment lock (PLAN.md §6.2, B.1, ADR-0001).\n"
        "# Generated by `leanfaith doctor --write-lock`; validated on every doctor run.\n"
    )
    body = yaml.safe_dump(
        canonical.model_dump(mode="json"), sort_keys=False, default_flow_style=False
    )
    lock_path.write_text(header + body, encoding="utf-8")
    return True, f"wrote {lock_path}"
