"""Experimental deterministic-v2 D0 runtime for LF-034 N14."""
...
N14RuleId = Literal["n14_negation_scope"]


class V2D0N14ExecutionError(ValueError):
    """The N14 profile, dispatch, or output violated its closed scope."""


class V2D0N14RuleBinding(StrictModel):
    family_id: Literal["n14_negation_scope"]
    rule_id: Literal["n14_negation_scope"]
    rule_version: Literal["1.0.0"]
    implementation_key: Literal["n14_negation_scope"]
    evidence_class: Literal["D0"]
    intended_error_types: tuple[Literal["E04", "E26"], ...]

    @model_validator(mode="after")
    def _closed_binding(self) -> V2D0N14RuleBinding:
        if not (self.family_id == self.rule_id == self.implementation_key == "n14_negation_scope"):
            raise ValueError("N14 binding identity fields must be identical")
        if self.intended_error_types != ("E04", "E26"):
            raise ValueError("N14 intended_error_types must be exactly [E04, E26]")
        return self


class V2D0N14ExecutionConfig(StrictModel):
    schema_version: Literal[1] = 1
    profile_id: Literal["deterministic_v2_d0_n14_experimental"]
    ...
    candidate_pool: Literal["deterministic_v2_d0_n14_experimental"]
    active_rules: tuple[V2D0N14RuleBinding, ...]
    ...
    require_exact_negation_scope_move: Literal[True] = True
    require_single_explicit_universal_binder: Literal[True] = True
    ...
    @model_validator(mode="after")
    def _closed_profile(self) -> V2D0N14ExecutionConfig:
        if tuple(binding.rule_id for binding in self.active_rules) != ("n14_negation_scope",):
            raise ValueError("the N14 profile must contain only N14")
        if self.required_candidate_views != (
            ...
            raise ValueError("N14 required_candidate_views changed")
        return self


def load_v2_d0_n14_execution_config(
    repo_root: Path | None = None,
    *,
    path: Path | None = None,
) -> LoadedConfig[V2D0N14ExecutionConfig]:
    root = find_repo_root(repo_root)
    resolved = (path or root / "configs/transformations/v2_d0_n14_experimental.yaml").resolve()
    if not resolved.is_relative_to(root.resolve()):
        raise V2D0N14ExecutionError("v2 N14 config escapes the repository")
    loaded = load_config(resolved, V2D0N14ExecutionConfig)
    portfolio = load_v2_portfolio(root)
    if loaded.config.portfolio_config_hash != portfolio.config_hash:
        raise V2D0N14ExecutionError("N14 profile does not bind the v2 portfolio")
    if (
        loaded.config.accepted_v1_effective_registry_hash
        != portfolio.config.accepted_v1.effective_registry_hash
    ):
        raise V2D0N14ExecutionError("N14 profile does not bind accepted v1 replay")
    designs = {item.family_id: item for item in portfolio.config.families}
    design = designs["n14_negation_scope"]
    if design.evidence_class != V2EvidenceClass.D0:
        raise V2D0N14ExecutionError("N14 is not a D0 design")
    if design.executable or design.draft_emission_authorized:
        raise V2D0N14ExecutionError("LF-031 design config was mutated into an executor")
    return loaded


class V2D0N14Runtime:
    """Code-owned unary dispatcher for only N14."""

    def __init__(self, loaded: LoadedConfig[V2D0N14ExecutionConfig]) -> None:
        self.loaded = loaded
        self.generation_config_hash = loaded.config_hash
        self.portfolio_hash = loaded.config.portfolio_config_hash
        self._rule = N14NegationScopeRule(
            generation_config_hash=loaded.config_hash,
            candidate_pool=loaded.config.candidate_pool,
        )

    @property
    def rule_ids(self) -> tuple[N14RuleId, ...]:
        return ("n14_negation_scope",)
    ...
    def execute(
        ...
        if rule_id != self._rule.rule_id:
            raise V2D0N14ExecutionError(f"rule {rule_id!r} is outside the N14 profile")
        if theorem.theorem_id != representation.theorem_id:
            raise V2D0N14ExecutionError("source theorem/representation lineage mismatch")
        if theorem.context_id != representation.context_id:
            raise V2D0N14ExecutionError("source theorem/representation context mismatch")
        ...
        for draft in drafts:
            try:
                ...
                raise V2D0N14ExecutionError("N14 emitted an invalid draft ID") from exc
            if (
                ...
                raise V2D0N14ExecutionError("N14 emitted a draft outside its binding")
...
def build_v2_d0_n14_runtime(
    repo_root: Path | None = None,
    *,
    path: Path | None = None,
) -> V2D0N14Runtime:
    return V2D0N14Runtime(load_v2_d0_n14_execution_config(repo_root, path=path))


__all__ = [
    "N14RuleId",
    "V2D0N14ExecutionConfig",
