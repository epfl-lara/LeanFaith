"""Experimental deterministic-v2 E2 runtime for LF-033 P16."""
...
P16RuleId = Literal["p16_conjunction_reassociation"]


class V2E2P16ExecutionError(ValueError):
    """The P16 profile, dispatch, or output violated its closed scope."""


class V2E2P16RuleBinding(StrictModel):
    family_id: Literal["p16_conjunction_reassociation"]
    rule_id: Literal["p16_conjunction_reassociation"]
    rule_version: Literal["1.0.0"]
    implementation_key: Literal["p16_conjunction_reassociation"]
    evidence_class: Literal["E2"]

    @model_validator(mode="after")
    def _closed_binding(self) -> V2E2P16RuleBinding:
        if not (
            self.family_id
            == self.rule_id
            ...
            raise ValueError("P16 binding identity fields must be identical")
        return self


class V2E2P16ExecutionConfig(StrictModel):
    schema_version: Literal[1] = 1
    profile_id: Literal["deterministic_v2_e2_p16_experimental"]
    ...
    candidate_pool: Literal["deterministic_v2_e2_p16_experimental"]
    active_rules: tuple[V2E2P16RuleBinding, ...]
    ...
    require_exact_root_conjunction_reassociation: Literal[True] = True
    require_distinct_conjunction_atoms: Literal[True] = True
    require_preserved_atom_order: Literal[True] = True
    ...
    @model_validator(mode="after")
    def _closed_profile(self) -> V2E2P16ExecutionConfig:
        if tuple(binding.rule_id for binding in self.active_rules) != (
            "p16_conjunction_reassociation",
        ):
            raise ValueError("the P16 profile must contain only P16")
        if self.required_candidate_views != (
            ...
            raise ValueError("P16 required_candidate_views changed")
        return self


def load_v2_e2_p16_execution_config(
    repo_root: Path | None = None,
    *,
    path: Path | None = None,
) -> LoadedConfig[V2E2P16ExecutionConfig]:
    root = find_repo_root(repo_root)
    resolved = (path or root / "configs/transformations/v2_e2_p16_experimental.yaml").resolve()
    if not resolved.is_relative_to(root.resolve()):
        raise V2E2P16ExecutionError("v2 P16 config escapes the repository")
    loaded = load_config(resolved, V2E2P16ExecutionConfig)
    portfolio = load_v2_portfolio(root)
    if loaded.config.portfolio_config_hash != portfolio.config_hash:
        raise V2E2P16ExecutionError("P16 profile does not bind the v2 portfolio")
    if (
        loaded.config.accepted_v1_effective_registry_hash
        != portfolio.config.accepted_v1.effective_registry_hash
    ):
        raise V2E2P16ExecutionError("P16 profile does not bind accepted v1 replay")
    designs = {item.family_id: item for item in portfolio.config.families}
    design = designs["p16_conjunction_reassociation"]
    if design.evidence_class != V2EvidenceClass.E2:
        raise V2E2P16ExecutionError("P16 is not an E2 design")
    if design.executable or design.draft_emission_authorized:
        raise V2E2P16ExecutionError("LF-031 design config was mutated into an executor")
    return loaded


class V2E2P16Runtime:
    """Code-owned unary dispatcher for only P16."""

    def __init__(self, loaded: LoadedConfig[V2E2P16ExecutionConfig]) -> None:
        self.loaded = loaded
        self.generation_config_hash = loaded.config_hash
        self.portfolio_hash = loaded.config.portfolio_config_hash
        self._rule = P16ConjunctionReassociationRule(
            generation_config_hash=loaded.config_hash,
            candidate_pool=loaded.config.candidate_pool,
        )

    @property
    def rule_ids(self) -> tuple[P16RuleId, ...]:
        return ("p16_conjunction_reassociation",)
    ...
    def execute(
        ...
        if rule_id != self._rule.rule_id:
            raise V2E2P16ExecutionError(f"rule {rule_id!r} is outside the P16 profile")
        if theorem.theorem_id != representation.theorem_id:
            raise V2E2P16ExecutionError("source theorem/representation lineage mismatch")
        if theorem.context_id != representation.context_id:
            raise V2E2P16ExecutionError("source theorem/representation context mismatch")
        ...
        for draft in drafts:
            try:
                ...
                raise V2E2P16ExecutionError("P16 emitted an invalid draft ID") from exc
            if (
                ...
                raise V2E2P16ExecutionError("P16 emitted a draft outside its binding")
...
def build_v2_e2_p16_runtime(
    repo_root: Path | None = None,
    *,
    path: Path | None = None,
) -> V2E2P16Runtime:
    return V2E2P16Runtime(load_v2_e2_p16_execution_config(repo_root, path=path))


__all__ = [
    "P16RuleId",
    "V2E2P16ExecutionConfig",
    "V2E2P16ExecutionError",
    "V2E2P16RuleBinding",
    "V2E2P16Runtime",
    "build_v2_e2_p16_runtime",
    "load_v2_e2_p16_execution_config",
]
