"""Experimental deterministic-v2 D0 runtime for LF-034 N13."""
...
from leanfaith.transforms.negatives.n13_witness_dependency import N13WitnessDependencyRule
...
N13RuleId = Literal["n13_witness_dependency"]


class V2D0N13ExecutionError(ValueError):
    """The N13 profile, dispatch, or output violated its closed scope."""


class V2D0N13RuleBinding(StrictModel):
    family_id: Literal["n13_witness_dependency"]
    rule_id: Literal["n13_witness_dependency"]
    rule_version: Literal["1.0.0"]
    implementation_key: Literal["n13_witness_dependency"]
    evidence_class: Literal["D0"]
    intended_error_types: tuple[Literal["E05", "E22"], ...]

    @model_validator(mode="after")
    def _closed_binding(self) -> V2D0N13RuleBinding:
        if not (
            self.family_id == self.rule_id == self.implementation_key == "n13_witness_dependency"
        ):
            raise ValueError("N13 binding identity fields must be identical")
        if self.intended_error_types != ("E05", "E22"):
            raise ValueError("N13 intended_error_types must be exactly [E05, E22]")
        return self


class V2D0N13ExecutionConfig(StrictModel):
    schema_version: Literal[1] = 1
    profile_id: Literal["deterministic_v2_d0_n13_experimental"]
    ...
    candidate_pool: Literal["deterministic_v2_d0_n13_experimental"]
    active_rules: tuple[V2D0N13RuleBinding, ...]
    ...
    require_exact_witness_dependency_swap: Literal[True] = True
    require_independent_witness_type: Literal[True] = True
    ...
    @model_validator(mode="after")
    def _closed_profile(self) -> V2D0N13ExecutionConfig:
        if tuple(binding.rule_id for binding in self.active_rules) != ("n13_witness_dependency",):
            raise ValueError("the N13 profile must contain only N13")
        if self.required_candidate_views != (
            ...
            raise ValueError("N13 required_candidate_views changed")
        return self


def load_v2_d0_n13_execution_config(
    repo_root: Path | None = None,
    *,
    path: Path | None = None,
) -> LoadedConfig[V2D0N13ExecutionConfig]:
    root = find_repo_root(repo_root)
    resolved = (path or root / "configs/transformations/v2_d0_n13_experimental.yaml").resolve()
    if not resolved.is_relative_to(root.resolve()):
        raise V2D0N13ExecutionError("v2 N13 config escapes the repository")
    loaded = load_config(resolved, V2D0N13ExecutionConfig)
    portfolio = load_v2_portfolio(root)
    if loaded.config.portfolio_config_hash != portfolio.config_hash:
        raise V2D0N13ExecutionError("N13 profile does not bind the v2 portfolio")
    if (
        loaded.config.accepted_v1_effective_registry_hash
        != portfolio.config.accepted_v1.effective_registry_hash
    ):
        raise V2D0N13ExecutionError("N13 profile does not bind accepted v1 replay")
    designs = {item.family_id: item for item in portfolio.config.families}
    design = designs["n13_witness_dependency"]
    if design.evidence_class != V2EvidenceClass.D0:
        raise V2D0N13ExecutionError("N13 is not a D0 design")
    if design.executable or design.draft_emission_authorized:
        raise V2D0N13ExecutionError("LF-031 design config was mutated into an executor")
    return loaded


class V2D0N13Runtime:
    """Code-owned unary dispatcher for only N13."""

    def __init__(self, loaded: LoadedConfig[V2D0N13ExecutionConfig]) -> None:
        self.loaded = loaded
        self.generation_config_hash = loaded.config_hash
        self.portfolio_hash = loaded.config.portfolio_config_hash
        self._rule = N13WitnessDependencyRule(
            generation_config_hash=loaded.config_hash,
            candidate_pool=loaded.config.candidate_pool,
        )

    @property
    def rule_ids(self) -> tuple[N13RuleId, ...]:
        return ("n13_witness_dependency",)
    ...
    def execute(
        ...
        if rule_id != self._rule.rule_id:
            raise V2D0N13ExecutionError(f"rule {rule_id!r} is outside the N13 profile")
        if theorem.theorem_id != representation.theorem_id:
            raise V2D0N13ExecutionError("source theorem/representation lineage mismatch")
        if theorem.context_id != representation.context_id:
            raise V2D0N13ExecutionError("source theorem/representation context mismatch")
        ...
        for draft in drafts:
            try:
                ...
                raise V2D0N13ExecutionError("N13 emitted an invalid draft ID") from exc
            if (
                ...
                raise V2D0N13ExecutionError("N13 emitted a draft outside its binding")
...
def build_v2_d0_n13_runtime(
    repo_root: Path | None = None,
    *,
    path: Path | None = None,
) -> V2D0N13Runtime:
    return V2D0N13Runtime(load_v2_d0_n13_execution_config(repo_root, path=path))


__all__ = [
    "N13RuleId",
    "V2D0N13ExecutionConfig",
