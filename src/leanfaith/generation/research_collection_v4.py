...
Collector v4 is a new envelope around the immutable v1 execution primitives.
It does not modify or reinterpret collector v1, v2, or v3 artifacts.  Unlike
collector v3, its scope is truthful for every preregistered Algebra or
cross-domain tranche rather than naming one particular ``cross_domain_s0``
run.
...
are separately versioned v4.  Raw collection performs no parsing, semantic
...
_HEX64 = r"^[0-9a-f]{64}$"
_PLAN_ID = r"^research_collection_plan_v4:[0-9a-f]{64}$"
_MANIFEST_ID = r"^research_collection_manifest_v4:[0-9a-f]{64}$"
_TRANCHE = r"^[a-z][a-z0-9_]*$"
_V4_ORCHESTRATION_ARTIFACT = "src/leanfaith/generation/research_collection_v4.py"
...
class ResearchCollectionV4Error(_v1.ResearchCollectionError):
    """The generic collection-v4 contract is invalid or incomplete."""


class ResearchCollectionV4ExecutionBlocked(
    ResearchCollectionV4Error,
    _v1.ResearchCollectionExecutionBlocked,
):
    """Execution was requested before every v4 prerequisite passed."""


class ResearchCollectionV4ArtifactConflict(
    ResearchCollectionV4Error,
    _v1.ResearchCollectionArtifactConflict,
):
    """A supposedly immutable v4 artifact contains different bytes."""


class ResearchCollectionV4Config(StrictModel):
    """Authorization for one preregistered closed-pool LF-021 tranche."""

    schema_version: Literal[4] = 4
    config_id: Literal["lf021_local_research_collection_v4"]
    tranche_id: str = Field(pattern=_TRANCHE)
    frozen_at: datetime.datetime
    artifact_class: Literal["research"] = "research"
    collection_scope: Literal["preregistered_closed_pool_three_family_tranche_v4"] = (
    ...
    def _coherent(self) -> Self:
        ...
        if len(self.families) != 3:
            raise ValueError("collection v4 requires exactly three model families")
        ...
        if any(
            ...
            raise ValueError("each v4 family requires sorted unique nonnegative seeds")
        ready = all(family.activation.status == "ready" for family in self.families)
        if self.status == "ready":
            if not self.execution_enabled or not ready:
                raise ValueError("ready v4 collection requires all family activations")
        elif self.execution_enabled or ready:
            raise ValueError("blocked v4 collection cannot enable all execution")
        if self.runtime.orchestration_adapter.artifact != _V4_ORCHESTRATION_ARTIFACT:
            raise ValueError("collection v4 must bind the v4 orchestration module")
        output_parts = PurePosixPath(self.outputs.root).parts
        if "v4" not in output_parts:
            raise ValueError("collection v4 output root must contain a v4 path component")
        if self.tranche_id not in output_parts:
            raise ValueError("collection v4 output root must contain the exact tranche_id")
        if "v4" not in PurePosixPath(self.outputs.preflight_report).stem:
            raise ValueError("collection v4 preflight report must be explicitly versioned v4")
        return self


class ScalableResearchSourceMatrixV4(StrictModel):
    """Tranche-specific identity matrix binding an immutable v2/v3 matrix."""

    schema_version: Literal[4] = 4
    matrix_id: str = Field(pattern=r"^local_research_source_matrix_v4:[0-9a-f]{64}$")
    tranche_id: str = Field(pattern=_TRANCHE)
    ...
    collection_authorization_source: Literal[
        "v4_config_and_original_pool_specific_overlap_evidence"
    ]
    ...
    def _coherent(self) -> Self:
        ...
        if self.source != expected_source:
            raise ValueError("v4 source matrix dialect and source disagree")
        ...
        if len(ids) != 3 or ids != tuple(sorted(set(ids))):
            raise ValueError("v4 source matrix requires three sorted families")
        if len(models) != len(set(models)):
            raise ValueError("v4 source-matrix model IDs must be unique")
        expected = "local_research_source_matrix_v4:" + hash_canonical(
            {"schema": "lf021_research_source_matrix_v4", **self.id_payload()}
        )
        if self.matrix_id != expected:
            raise ValueError("v4 source matrix ID does not match payload")
        return self


class ResearchCollectionPlanV4(StrictModel):
    """Deterministic plan for one arbitrary preregistered v4 tranche."""
    ...
    def _identity_and_counts(self) -> Self:
        ...
        if len(family_ids) != self.family_count or family_ids != tuple(sorted(set(family_ids))):
            raise ValueError("v4 plan requires three sorted unique family bindings")
        if list(self.seed_count_by_family) != sorted(self.seed_count_by_family):
            raise ValueError("v4 seed counts must be sorted")
        if set(self.seed_count_by_family) != set(family_ids):
            raise ValueError("v4 seed-count families differ from family bindings")
        if any(count < 1 for count in self.seed_count_by_family.values()):
            raise ValueError("every v4 family must have at least one seed")
        ...
        if self.expected_candidate_count != expected or len(self.invocations) != expected:
            raise ValueError("v4 invocation denominator does not reconcile")
        invocation_ids = tuple(item.invocation_id for item in self.invocations)
        if invocation_ids != tuple(sorted(set(invocation_ids))):
            raise ValueError("v4 plan invocations must be sorted and unique")
        ...
        for family_id, seed_count in self.seed_count_by_family.items():
            if actual[family_id] != self.problem_count * seed_count:
                raise ValueError("v4 family invocation count does not reconcile")
        if {item.problem_record_id for item in self.invocations} != set(self.problem_record_ids):
            raise ValueError("v4 invocation problems differ from frozen pool")
        expected_id = "research_collection_plan_v4:" + hash_canonical(
            {"schema": "lf021_research_collection_plan_v4", **self.id_payload()}
        )
        if self.plan_id != expected_id:
            raise ValueError("plan_id does not match frozen v4 plan payload")
        return self


class ResearchCollectionPreflightReportV4(StrictModel):
    """Model-free replay report for one generic v4 tranche."""

    schema_version: Literal[4] = 4
    report_kind: Literal["lf021_local_research_collection_preflight_v4"]
    ...
    def _coherent(self) -> Self:
        if not self.checks or not all(self.checks.values()):
            raise ValueError("collection v4 preflight requires every model-free check")
        ...
        if (
            ...
            raise ValueError("v4 preflight denominator does not reconcile")
        return self


class ResearchCollectionManifestV4(StrictModel):
    """Complete terminal accounting for one collector-v4 plan."""
    ...
    def _coherent(self) -> Self:
        ...
        if (
            ...
            raise ValueError("v4 manifest accounting does not reconcile")
        ...
        expected_id = "research_collection_manifest_v4:" + hash_canonical(
            {"schema": "lf021_research_collection_manifest_v4", **self.id_payload()}
        )
        if self.manifest_id != expected_id:
            raise ValueError("manifest_id does not match v4 manifest payload")
        return self


@dataclass(frozen=True, slots=True)
class VerifiedResearchFamilyActivationV4:
...
@dataclass(frozen=True, slots=True)
class LoadedResearchCollectionV4:
    config: LoadedConfig[ResearchCollectionV4Config]
    problems: tuple[ProblemPoolRecord, ...]
    context: ContextRecord
    source_matrix: ScalableResearchSourceMatrixV4
    pool_evidence: _v3.ScalableProblemPoolEvidence
    qualifications: Mapping[str, LoadedConfig[LocalQualificationConfig]]
    activation_evidence: Mapping[str, VerifiedResearchFamilyActivationV4]
    plan: ResearchCollectionPlanV4
    preflight: ResearchCollectionPreflightReportV4


@dataclass(frozen=True, slots=True)
class ResearchCollectionRunV4:
    output_directory: Path
    plan_path: Path
    manifest_path: Path
    manifest: ResearchCollectionManifestV4
    terminals: tuple[_v1.ResearchCollectionTerminal, ...]


class ResearchInvocationExecutorV4(Protocol):
...
def _load_source_matrix(
    path: Path,
) -> ScalableResearchSourceMatrixV4:
    raw = load_yaml_mapping(path)
    return ScalableResearchSourceMatrixV4.model_validate(raw)
...
def _verify_activation_v4(
    *,
    family: _v1.ResearchCollectionFamily,
    qualification: LoadedConfig[LocalQualificationConfig],
    config: ResearchCollectionV4Config,
    pool_evidence: _v3.ScalableProblemPoolEvidence,
    problems: tuple[ProblemPoolRecord, ...],
    repo_root: Path,
) -> VerifiedResearchFamilyActivationV4:
    ...
    try:
        if dialect == "gate3_algebra_operational_v1":
            ...
            return VerifiedResearchFamilyActivationV4(
        ...
        raise ResearchCollectionV4Error(
            f"v4 activation replay failed for {family.family_id}"
        ) from exc
...
def _load_expansion_authorization(
    *,
    ...
    try:
        ...
        raise ResearchCollectionV4Error("invalid expansion decision artifact") from exc
    policy = load_tranche_expansion_policy(policy_path).config
    if (
    ...
    if tuple(item.tranche_id for item in decision.observations) != expected_prefix:
        raise ResearchCollectionV4Error("expansion decision observations differ from policy prefix")
    tranches = {item.tranche_id: item for item in policy.tranches}
    for observation in decision.observations:
        ...
        replayed = load_postprocess_observation(
            repo_root=repo_root,
            policy=policy,
            tranche=tranches[observation.tranche_id],
            manifest_path=manifest_path,
        )
        if (
            replayed.manifest.manifest_id != observation.manifest_id
            or replayed.manifest.input_binding_hash != observation.input_binding_hash
        ):
            raise ResearchCollectionV4Error("expansion observation replay differs from decision")
    return decision, policy


def _build_plan(
    *,
    loaded_config: LoadedConfig[ResearchCollectionV4Config],
    config_path: Path,
    ...
    plan_id = "research_collection_plan_v4:" + hash_canonical(
        {"schema": "lf021_research_collection_plan_v4", **payload}
    )
    return ResearchCollectionPlanV4.model_validate({"plan_id": plan_id, **payload})


def load_research_collection_v4(
    config_path: Path,
    *,
    repo_root: Path,
) -> LoadedResearchCollectionV4:
    """Replay a generic v4 tranche without importing GPU libraries."""
    ...
    try:
        ...
        raise ResearchCollectionV4Error("v4 config must remain in the repository") from exc
    loaded = load_config(config_path, ResearchCollectionV4Config)
    ...
    if (
        ...
        raise ResearchCollectionV4Error("v4 config differs from expansion-selected tranche")
    artifact_bindings = (
    ...
    try:
        ...
        raise ResearchCollectionV4Error("v4 closed-pool validation failed") from exc
    ...
    if (
        ...
        raise ResearchCollectionV4Error("v4 problem, context, and import-header bindings disagree")
    ...
    if (
        ...
        raise ResearchCollectionV4Error("v4 source matrix differs from exact pool")
    ...
    v4_families = tuple((item.family_id, item.model, item.revision) for item in matrix.families)
    if (
        ...
        or legacy_families != v4_families
        or legacy_matrix.heldout.model_dump(mode="json") != matrix.heldout.model_dump(mode="json")
    ):
        raise ResearchCollectionV4Error("v4 source matrix differs from immutable base matrix")
    ...
    for family in config.families:
        ...
        if (
            ...
            raise ResearchCollectionV4Error(
                f"v4 family differs from source-matrix pin: {family.family_id}"
            )
    ...
    activation_evidence = {
        family.family_id: _verify_activation_v4(
            family=family,
    ...
    if {evidence.overlap_schema for evidence in activation_evidence.values()} != {
        ...
        raise ResearchCollectionV4Error("v4 overlap evidence version is inconsistent")
    ...
    preflight = ResearchCollectionPreflightReportV4(
        report_kind="lf021_local_research_collection_preflight_v4",
        ...
        checks={
            ...
            "tranche_scope_and_v4_outputs_are_truthful": True,
            "zero_labels_and_zero_gate_claims": True,
        },
        blocking_prerequisites=blockers,
    )
    return LoadedResearchCollectionV4(
        config=loaded,
        problems=problems,
        context=context,
...
def write_preflight_report_v4(
    loaded: LoadedResearchCollectionV4,
    *,
    repo_root: Path,
) -> tuple[Path, str]:
    """Persist or replay the exact model-free v4 preflight."""
...
def execute_research_collection_v4(
    loaded: LoadedResearchCollectionV4,
    *,
    repo_root: Path,
    executor: ResearchInvocationExecutorV4,
    clock: _v1.Clock = lambda: datetime.datetime.now(datetime.UTC),
) -> ResearchCollectionRunV4:
    """Execute or deterministically resume one terminal per frozen invocation."""

    if not loaded.preflight.execution_ready:
        raise ResearchCollectionV4ExecutionBlocked(
            "research collection v4 is blocked: "
    ...
    for family_id in sorted(binding_by_family):
        ...
        for invocation in family_invocations:
            ...
            if (
                ...
                raise ResearchCollectionV4ArtifactConflict(
    ...
    if len(terminals) != loaded.plan.expected_candidate_count:
        raise ResearchCollectionV4ArtifactConflict(
            "v4 run did not produce one terminal per invocation"
        )
    ...
    manifest = ResearchCollectionManifestV4.model_validate({"manifest_id": manifest_id, **payload})
    manifest_path = root / "manifest.json"
    _v1._write_immutable(
        manifest_path,
        _v1._canonical_record_bytes(manifest),
    )
    return ResearchCollectionRunV4(
...
def derive_research_collection_v4_config(
    *,
    ...
    """Create an immutable v4 config from a bound v2 or v3 pool config.

    This is the decision-complete tranche contract.  It changes only tranche
    identity, frozen seeds, v4 orchestration/output bindings, and versioned
    ...
    for path, label in (
        ...
        try:
            ...
            raise ResearchCollectionV4Error(f"v4 {label} config must remain in repository") from exc
    decision_binding = _v1.ResearchArtifactBinding(
    ...
    if version == 2:
        ...
        raise ResearchCollectionV4Error("base config must be collector v2 or v3")
    ...
    if (
        ...
        raise ResearchCollectionV4Error("base config does not bind the expansion-selected pool")
    ...
    for family in cast(list[dict[str, object]], document["families"]):
        ...
        if not seeds or seeds != tuple(sorted(set(seeds))) or any(seed < 0 for seed in seeds):
            raise ResearchCollectionV4Error("each family requires sorted unique nonnegative seeds")
        family["seeds"] = list(seeds)

    module_path = root / _V4_ORCHESTRATION_ARTIFACT
    matrix_payload: dict[str, object] = {
        ...
        "rules": [
            ...
            "activation remains in the v4 config and exact pool-specific overlap records",
            "model output creates no semantic label, supervision admission, or Gate claim",
        ],
    }
    matrix_id = "local_research_source_matrix_v4:" + hash_canonical(
        {"schema": "lf021_research_source_matrix_v4", **matrix_payload}
    )
    matrix = ScalableResearchSourceMatrixV4.model_validate(
    ...
    runtime["orchestration_adapter"] = {
        "artifact": _V4_ORCHESTRATION_ARTIFACT,
        "sha256": hash_file(module_path),
    }
    document.update(
        {
            "schema_version": 4,
            "config_id": "lf021_local_research_collection_v4",
            "tranche_id": tranche_id,
            "frozen_at": frozen_at.isoformat().replace("+00:00", "Z"),
            "collection_scope": ("preregistered_closed_pool_three_family_tranche_v4"),
            "expansion_decision": decision_binding.model_dump(mode="json"),
            "expansion_policy": policy_binding.model_dump(mode="json"),
            "outputs": {
                "root": (f"data/raw/real_outputs/{pool_slug}/v4/{tranche_id}/local_collection"),
                "preflight_report": (
                    "reports/generation/"
                    f"lf021_local_research_collection_preflight_{tranche_id}_v4.json"
                ),
            },
            "rules": [
                ...
                (
                    ...
                    "manifest, tranche, and roots are v4"
                ),
            ],
        }
    )
    config = ResearchCollectionV4Config.model_validate(document)
    digest = _v1._write_immutable(
        destination,
...
__all__ = [
    ...
    "ResearchCollectionManifestV4",
    "ResearchCollectionPlanV4",
    "ResearchCollectionPreflightReportV4",
    "ResearchCollectionRunV4",
    "ResearchCollectionTerminal",
    "ResearchCollectionV4ArtifactConflict",
    "ResearchCollectionV4Config",
    "ResearchCollectionV4Error",
    "ResearchCollectionV4ExecutionBlocked",
