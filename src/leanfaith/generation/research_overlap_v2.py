"""Scalable checkpoint-to-public-pool overlap records for LF-021.

Revision 1 of the overlap record intentionally bound exactly three source
introductions.  This revision removes only that cardinality restriction while
preserving the conservative interpretation: temporal non-overlap permits raw
collection over the exact bound pool, but it is never evidence that a
...
_HEX40 = r"^[0-9a-f]{40}$"
_HEX64 = r"^[0-9a-f]{64}$"
_OVERLAP_ID = r"^research_overlap_v2:[0-9a-f]{64}$"


class ResearchFamilyOverlapRecordV2(StrictModel):
    """Cautious authorization for one arbitrary-size frozen public pool."""

    schema_version: Literal[2] = 2
    record_kind: Literal["lf021_research_family_overlap_v2"] = "lf021_research_family_overlap_v2"
    ...
    def _coherent(self) -> Self:
        ...
        expected = "research_overlap_v2:" + hash_canonical(
            {"schema": "lf021_research_family_overlap_v2", **self.id_payload()}
        )
    ...
    def create(cls, **values: object) -> Self:
        ...
        payload = {
            "schema_version": 2,
            "record_kind": "lf021_research_family_overlap_v2",
            **normalized,
        }
        provisional = cls.model_construct(
            None,
            overlap_id="research_overlap_v2:" + ("0" * 64),
            **payload,
        )
        complete_payload = provisional.model_dump(mode="json", warnings=False)
        complete_payload.pop("overlap_id")
        overlap_id = "research_overlap_v2:" + hash_canonical(
            {"schema": "lf021_research_family_overlap_v2", **complete_payload}
        )
        return cls.model_validate({"overlap_id": overlap_id, **complete_payload})


__all__ = ["ResearchFamilyOverlapRecordV2"]
